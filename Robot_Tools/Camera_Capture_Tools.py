...
#TODO:These values might need adjusting based on lighting
def detect_and_annotate_blocks(frame, area_threshold=1500):
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    color_ranges = {
        "blue":   [(np.array([100, 140, 80]), np.array([130, 255, 255]))],
    ...
    for color_name, ranges in color_ranges.items():
        ...
        for cnt in contours:
            ...
            if area < area_threshold or area > 8000:
                continue

            x, y, w, h = cv2.boundingRect(cnt)

            # Reject non-square shapes (blocks are roughly square)
            aspect = w / float(h)
            ...
            put_text(frame, f"{label_name} ({cx},{cy}), cnt_area:{area}", (x, y - 10))
