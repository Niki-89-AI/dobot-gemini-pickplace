...
from Robot_Tools.Robot_Motion_Tools import (  # noqa: E402
...
def pick_and_place_block(
    ...
    try:
        ...
        # TODO(student):
        # Build a dictionary called label_to_uv.
        # Each key should be a detected block label like "blue1".
        # Each value should be a tuple (x, y) containing the pixel coordinates.
        label_to_uv = {}
        ...
        # TODO(student):
        # Check whether source_label and target_label exist in label_to_uv.
        # If either is missing, return an error dictionary with:
        #   - "error"
        #   - "available_labels"
        if source_label not in label_to_uv or target_label not in label_to_uv:
            return {
                "error": f"Source or target label not found: {source_label}, {target_label}",
                "available_labels": list(label_to_uv.keys())
            }


        #NOTE label_to_uv must be created above before using it here
        ...
        steps.append({"step": "return_to_home"})
